...
C1 = zeros(N_Realizations, 101);
C2 = zeros(N_Realizations, 101);
C3 = zeros(N_Realizations, 101);
C1b = zeros(N_Realizations, 101);
C2b = zeros(N_Realizations, 101);
C3b = zeros(N_Realizations, 101);
C1c = zeros(N_Realizations, 101);
C2c = zeros(N_Realizations, 101);
C3c = zeros(N_Realizations, 101);
...
for Realization = 1 : N_Realizations
    ...
    for iterrrr = 1:20
        ...
        %% bi-directional training
            ...
            [v11b, v12b, v13b, v21b, v22b, v23b, v31b, v32b, v33b] = MSE_b(Z11, Z12, Z13, Z21, Z22, Z23, Z31, Z32, Z33, g1b, g2b, g3b, n0);
            [v11c, v12c, v13c, v21c, v22c, v23c, v31c, v32c, v33c] = MSE_b(Z11, Z12, Z13, Z21, Z22, Z23, Z31, Z32, Z33, g1c, g2c, g3c, n0);
            v12c = [0;0];
            v13c = [0;0];
            v21c = [0;0];
            v23c = [0;0];
            ...
            [g1b, g2b, g3b] = MSE_f(H11, H12, H13, H21, H22, H23, H31, H32, H33, v11b, v12b, v13b, v21b, v22b, v23b, v31b, v32b, v33b, n0);
            [g1c, g2c, g3c] = MSE_f(H11, H12, H13, H21, H22, H23, H31, H32, H33, v11c, v12c, v13c, v21c, v22c, v23c, v31c, v32c, v33c, n0);
            ...
            C1(Realization,kkk ) = abs(log2(1+SINR1));
            C2(Realization,kkk ) = abs(log2(1+SINR2));
            C3(Realization,kkk) = abs(log2(1+SINR3));
            ...
            C1b(Realization,kkk) = abs(log2(1+SINR1b));
            C2b(Realization,kkk) = abs(log2(1+SINR2b));
            C3b(Realization,kkk) = abs(log2(1+SINR3b));
            ...
            C1c(Realization, kkk) = abs(log2(1+SINR1c));
            C2c(Realization, kkk) = abs(log2(1+SINR2c));
            C3c(Realization, kkk) = abs(log2(1+SINR3c));
...
x=linspace(10^(-3),1+10^(-3),101);
p1=semilogx(x, mean(C1)+mean(C2)+mean(C3),'o');
hold on
p2=semilogx(x, mean(C1b)+mean(C2b)+mean(C3b),'*');
p3=semilogx(x, mean(C1c)+mean(C2c)+mean(C3c),'s');
